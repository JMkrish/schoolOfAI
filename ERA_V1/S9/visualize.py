...
def print_samples(loader, class_map, count=16):
    ...
    Args:
        ...
        class_map (list): mapping for class names
    ...
    for imgs, labels in loader:
        for i in range(count):
            ...
            ax.set_title(f"{class_map[labels[i]]}")
...
def print_class_scale(loader, class_map):
    ...
    """
    labels_count = {
        k: v for k, v in zip(range(0, len(class_map)), [0] * len(class_map))
    }
    ...
    plt.bar(class_map, values, width=0.5)
    plt.legend(labels=["Samples Per Class"])
    for l in range(len(class_map)):
...
def plot_incorrect_predictions(predictions, classes, count=10):
    """Plot Incorrect predictions

    Args:
        predictions (list): List of all incorrect predictions
        classes (list): Lable mapping
    ...
    if not count % 5 == 0:
        print("Count should be a multiple of 5")
        return

    fig = plt.figure(figsize=(10, 5))

    for i, (d, t, p) in enumerate(predictions):
...
def plot_model_comparison(trained_models, epochs=20):
    ...
    plt_args = {"bn": ("b", "BN loss"), "gn": ("r", "GN loss"), "ln": ("m", "LN loss")}

    plt.subplot(1, 2, 1)
    for model in trained_models:
        color, label = plt_args[model.norm]
        plt.plot(range(epochs), model.test_losses, color, label=f"{label} loss")
    ...
    for model in trained_models:
        color, label = plt_args[model.norm]
        plt.plot(range(epochs), model.test_acc, color, label=f"{label} Accuracy")
